...
from PyQt6.QtGui import QFont,QIcon,QImage,\
    QPainter,QBrush,QColor,QTextCursor,QPolygonF
from PyQt6.QtCore import QObject,pyqtSignal,pyqtSlot,\
    Qt,QRectF,QPointF,QEasingCurve,QPropertyAnimation
from PyQt6.QtWidgets import QApplication,QMessageBox,\
    QInputDialog,QFileDialog,QLabel,QWidget,QFrame,\
    QMainWindow,QGraphicsBlurEffect,QColorDialog,\
    QVBoxLayout,QHBoxLayout,QPushButton,QTextEdit,QCheckBox,\
...
class RwConfig:
    apikey=blopen=blradius=bgcolor=bgtheme=qradius=\
...
class CheckUpdate:
    ...
    def checkUpdate(self,qt=None):
        try:
            ...
            l14.setText('云端版本:'+str(new_version))
            l15.setText('检查时间:'+lasttime)
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        link.clicked.connect(MainWindow.imagethread)
        link.setIcon(QIcon('images/link.png'))
        link.setToolTip('选择图片以使用visual模型')
        ...
        self.settingw=SettingWindow(self,self.historyw,center,label,l1,self.t1,t2,self.b2,self.b3,self.historyw.center,self.historyw.label)  
...
class SettingWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        def settheme():
            ...
            if color=='default':
                ...
                if color=='white':
                    ...
                    self.mwbg.setStyleSheet('background:white')
                    center.setStyleSheet('background:white')
                    self.tq.setStyleSheet(f'background:rgba(0,0,0,0.5);border-radius:{RwConfig.qradius}px')
                    self.ba.setStyleSheet('background:rgba(0,0,0,0.5);border-radius:15px')
                else:
        ...
        global l15
        l15=QLabel('检查时间:'+str(RwConfig.lasttime))
        ...
        b18.clicked.connect(lambda:SettingWindow.updatethread(True,b18))
...
def main():
    if RwConfig.apikey=='':messagebox.showdialog()
